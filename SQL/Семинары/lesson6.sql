...
-- вызов процедуры
CALL sp_user_add('New', 'User', 'new_user12@mail.com', 9110001122, 'Moscow', -1, '1998-01-01', @tran_result); 
SELECT @tran_result;
...
-- обновим данные в базе, чтобы появились пользователи из одного города все, кто младше - будут в городе Adriannaport
UPDATE profiles
...
-- общий город
	...
	WHERE p1.user_id = for_user_id AND p2.user_id <> for_user_id  -- <> ранвосильно != (не равно)
    UNION -- объединяем, все одинаковые значения схлопываются
...
-- друзья друзей
	...
	ORDER BY rand() -- вывод случайных значений в задаче - Семинар 6 01 час. 37 мин.
	LIMIT 5;
...
RETURNS FLOAT READS SQL DATA -- без этого  выражения READS SQL DATA - ошибка DETERMENISTIC
BEGIN
	...
	/* 
	SET requests_from_user = ( -- присваиваем переменной значение, можно так. а можно - смотри ниже
	SELECT COUNT(*) 
	FROM friend_requests
	...
	SELECT COUNT(*)
	INTO  requests_from_user -- посылаем в переменную результат выборки
	FROM friend_requests
	WHERE initiator_user_id = check_user_id; 
	
	RETURN requests_to_user / requests_from_user; -- возвращаем результат
END//
DELIMITER ;

-- вызов функции
SELECT friendship_direction(1);
SELECT truncate(friendship_direction(1), 2)*100 AS `user popularity`; -- truncate обрезает до нужного количества знаков


-- ЦИКЛЫ -- добавим в таблицу дату обновления профиля
ALTER TABLE `profiles`
ADD COLUMN time_update DATETIME ON UPDATE NOW(); 


DROP PROCEDURE IF EXISTS sp_data_analysis; -- процедура анализа данных
DELIMITER //
...
begin
    IF NEW.birthday > NOW() THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Ошибка - Обновление отменено. Дата рождения не может быть больше текущей даты!';
    END IF;
END//
DELIMITER ;
