...
-- Выводим таких пользователей (для проверки):
SELECT p.*, l.* FROM profiles p 
LEFT JOIN likes l ON p.user_id=l.user_id
WHERE (YEAR(NOW())-YEAR(p.birthday)) < 12;

-- Подсчитываем:
SELECT COUNT(*) as 'Количество лайков' FROM profiles p 
LEFT JOIN likes l ON p.user_id=l.user_id
WHERE (YEAR(NOW())-YEAR(p.birthday)) < 12;

-- Задача 2
-- Определить кто больше поставил лайков (всего): мужчины или женщины
SELECT gender AS 'Больше поставили', COUNT(*) AS 'Количество лайков' FROM profiles p 
JOIN likes l ON p.user_id=l.user_id
-- группируем по 'gender', сортируем по 'COUNT' по возрастанию
GROUP BY gender ORDER BY COUNT(*)
-- выводим вторую строку, максимум из двух строк
LIMIT 1, 1;

-- Задача 3
-- Вывести всех пользователей, которые не отправляли сообщения.
-- Выводим таких пользователей (с сообщением NULL) - для проверки
